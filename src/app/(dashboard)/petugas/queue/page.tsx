  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div custom={1} initial="hidden" animate="visible" variants={fadeIn}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={stat.label} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <motion.div custom={1.5} initial="hidden" animate="visible" variants={fadeIn}>
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Filter Poli:</span>
        ...
        {loading ? (
            ...
            {[1, 2].map((i) => (
              <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-6 animate-pulse">
                <div className="flex items-center gap-3 mb-4">
                  <div className="h-10 w-10 bg-slate-200 dark:bg-slate-600 rounded-xl" />
                  <div className="h-4 w-32 bg-slate-200 dark:bg-slate-600 rounded" />
                </div>
                <div className="space-y-3">
                  {[1, 2, 3].map((j) => (
                    <div key={j} className="h-16 bg-slate-100 dark:bg-slate-700 rounded-xl" />
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="space-y-4">
            {Object.entries(grouped).map(([poliName, items]) => {
              ...
              return (
                <div key={poliName} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">{poliName}</h3>
                      ...
                      >
                        <div className="px-5 pb-5 space-y-2">
                          {items.map((q) => {
                            ...
                            return (
                              <div key={q.id} className="flex items-center justify-between rounded-xl border border-slate-100 dark:border-slate-700 p-4 hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-all">
                                <div className="flex items-center gap-4">
                                  <span className={`flex h-12 w-12 items-center justify-center rounded-xl text-sm font-bold shadow-sm ${
                                    q.status === 'dipanggil' ? 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white animate-pulse shadow-blue-200 dark:shadow-blue-900/30' :
                                    q.status === 'sedang_diperiksa' ? 'bg-gradient-to-br from-teal-500 to-emerald-600 text-white shadow-teal-200 dark:shadow-teal-900/30' :
                                    q.status === 'selesai' ? 'bg-gradient-to-br from-emerald-500 to-green-600 text-white shadow-emerald-200 dark:shadow-emerald-900/30' :
                                    'bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 text-slate-700 dark:text-slate-200 shadow-slate-200 dark:shadow-slate-900/30'
                                  }`}>
                                    {q.queue_number}
                                  </span>
                                  <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                      <span className="text-[11px] text-slate-500 dark:text-slate-400">RM: {q.patient?.medical_record_number || '-'}</span>
