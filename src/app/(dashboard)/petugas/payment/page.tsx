...
export default function PetugasPaymentPage() {
  ...
  const [paymentMethod, setPaymentMethod] = React.useState<'tunai' | 'transfer'>('tunai');
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      await logAudit('UPDATE', 'payments', selectedPayment.id, { status: 'belum_bayar' }, { status: 'dibayar', payment_method: paymentMethod });
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0c3b33] via-[#0f4a3f] to-[#1a5c4f] p-6 text-white shadow-xl shadow-teal-900/20">
          ...
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/60 text-xs mt-0.5">Proses pembayaran pasien</p>
            </div>
          </div>
        </div>
      </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {selectedPayment.status === 'belum_bayar' && (
                  <>
                    <div>
                      <label className="text-sm font-medium text-slate-700">Metode Pembayaran</label>
                      <Select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as 'tunai' | 'transfer')} className="mt-1">
                    ...
                    <div className="flex gap-3 pt-4">
                      <Button onClick={() => setShowDetail(false)} variant="outline" className="flex-1">
                        Batal
                      </Button>
                      <Button onClick={handleProcessPayment} disabled={processing} className="flex-1 bg-gradient-to-r from-teal-500 to-emerald-500">
                        {processing ? (
                          ...
                          <>
                            <CheckCircle2 className="h-4 w-4 mr-2" /> Bayar
                          </>
                        )}
                      </Button>
                    </div>
                ...
                {selectedPayment.status === 'dibayar' && (
                  <div className="p-4 bg-green-50 rounded-xl text-center">
                    <CheckCircle2 className="h-8 w-8 text-green-500 mx-auto mb-2" />
                    <p className="text-sm font-semibold text-green-700">Sudah Dibayar</p>
                    <p className="text-xs text-green-600 mt-1">
                      {selectedPayment.payment_method && `via ${selectedPayment.payment_method}`}
                      {selectedPayment.paid_at && ` pada ${format(new Date(selectedPayment.paid_at), 'dd MMM yyyy, HH:mm', { locale: id })}`}
                    </p>
                  </div>
                )}
              </div>
